# Contributing to Do React App A Create

Loving Do React App A Create and want to get involved? Thanks! There are plenty of ways you can help.
...
Normally, `npm start` runs on port `3000`, and this is not explicitly configurable. However, some environments like cloud IDEs want the programs to run on a specific port to serve their output. We want to play well with different environments, so Do React App A Create reads `PORT` environment variable and prefers it when it is specified. The trick is that we know cloud IDEs already specify it automatically, so there is no need for the user to do anything. Do React App A Create relies on heuristics to do the right thing depending on environment.
...
We prefer to add interactivity to the command line interface rather than add configuration flags. For example, `npm start` will attempt to run with port `3000` by default, but it may be busy. Many other tools just fail in this case and ask that you pass a different port, but Do React App A Create will display a prompt asking if you’d like to run the app on the next available port.

Another example of interactivity is `npm test` watcher interface. Instead of asking people to pass command line flags for switching between test runner modes or search patterns, we print a hint with keys that you can press during the test session to instruct watcher what to do. Jest supports both flags and interactive CLI but Do React App A Create prefers long-running sessions to keep user immersed in the flow over short-running sessions with different flags.
...
Please **ask first** if somebody else is already working on this or the core developers think your feature is in-scope for Do React App A Create. Generally always have a related issue with discussions for whatever you are including.

Please also provide a **test plan**, i.e. specify how you verified that your addition works.

## Folder Structure of Do React App A Create

`do-react-app-a-create` is a monorepo, meaning it is divided into independent sub-packages.<br>
These packages can be found in the [`packages/`](https://github.com/peterkhayes/do-react-app-a-create/tree/master/packages) directory.
...
#### [babel-preset-react-app](https://github.com/peterkhayes/do-react-app-a-create/tree/master/packages/babel-preset-react-app)
...
This package is enabled by default for all `do-react-app-a-create` scaffolded applications.

#### [do-react-app-a-create](https://github.com/peterkhayes/do-react-app-a-create/tree/master/packages/do-react-app-a-create)

The global CLI command code can be found in this directory, and shouldn't often be changed. It should run on Node 0.10+.

#### [eslint-config-react-app](https://github.com/peterkhayes/do-react-app-a-create/tree/master/packages/eslint-config-react-app)

This package contains a conservative set of rules focused on making errors apparent and enforces no style rules.<br>
This package is enabled by default for all `do-react-app-a-create` scaffolded applications.

#### [react-dev-utils](https://github.com/peterkhayes/do-react-app-a-create/tree/master/packages/react-dev-utils)
...
#### [react-scripts](https://github.com/peterkhayes/do-react-app-a-create/tree/master/packages/react-scripts)
...
1. Clone the repo with `git clone https://github.com/peterkhayes/do-react-app-a-create`

2. Run `yarn` in the root `do-react-app-a-create` folder.
...
```
yarn do-react-app-a-create my-app
cd my-app
```
...
3. In most releases, only `react-scripts` needs to be released. If you don’t have any changes to the `packages/do-react-app-a-create` folder, you don’t need to bump its version or publish it (the publish script will publish only changed packages).
4. Note that files in `packages/do-react-app-a-create` should be modified with extreme caution. Since it’s a global CLI, any version of `do-react-app-a-create` (global CLI) including very old ones should work with the latest version of `react-scripts`.
5. Run `yarn compile:lockfile`. The command will generate an updated lockfile in `packages/do-react-app-a-create` that should be committed.
